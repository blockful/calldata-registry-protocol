...
import type { DecodedAction, DecodedCall, DecodedParam, JsonValue } from "@/lib/calldataDecoder";
  ...
  return (
    <div className="space-y-3">
      <div>
        ...
        <span className="font-mono text-white/70">{call.signature}</span>
      </div>
      <div>
        <span className="text-white/40">Source </span>
        <span className="font-mono text-white/50">
          {call.source} / {call.confidence}
        </span>
      </div>
      {call.params.length > 0 ? (
        <div className="space-y-2">
          <span className="text-white/40 block">Parameters</span>
          <div className="space-y-2">
            {call.params.map((param, index) => (
              <DecodedParamRow key={`${param.name}-${index}`} param={param} />
            ))}
          </div>
        </div>
      ) : null}
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-baseline gap-x-2 gap-y-1">
        <span className="font-mono text-xs text-white/30">{param.type}</span>
        <span className="text-white/60">{param.name || "(unnamed)"}</span>
        {!hasChildren && (
      ...
      {param.decodedBytes && (
        <div className="mt-3 bg-white/[0.03] px-3 py-2">
...
function DecodedActionPanel({
  ...
  if (isLoading) {
    return <div className="text-white/30">Decoding calldata...</div>;
  }

  if (isError) {
    return <div className="text-white/30">Decoder unavailable.</div>;
  }

  if (!action) {
    return <div className="text-white/30">Decoder has no result.</div>;
  }

  if (action.status === "decoded" && action.decoded) {
    return <DecodedCallBlock call={action.decoded} />;
  }

  if (action.status === "empty" && action.decoded) {
    return <DecodedCallBlock call={action.decoded} />;
  }
      ...
      <div className="space-y-2">
        ...
        <div className="flex flex-wrap gap-2">
          {(action.candidates ?? []).map((candidate) => (
            <span
              key={candidate}
              className="font-mono text-xs text-white/50 bg-white/[0.04] px-2 py-1"
            >
              {candidate}
            </span>
          ))}
  ...
  const draftActions = useMemo(
    () =>
      detail?.targets?.map((target, i) => ({
        target,
        value: detail.values?.[i] ?? "0",
        calldata: detail.calldatas?.[i] ?? "0x",
      })) ?? [],
    [detail]
  );
  const {
