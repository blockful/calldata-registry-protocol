...
contract Deploy is Script {
    ...
    function run() external returns (address deployed) {
        bytes memory creationCode = type(CalldataDraft).creationCode;
        ...
        console.log("CalldataDraft deployed at:", deployed);
    }

    /// @notice Simple deployment using `new` (for local testing on Anvil)
    function deploySimple() external returns (CalldataDraft) {
        vm.startBroadcast();
        CalldataDraft registry = new CalldataDraft();
        vm.stopBroadcast();
        console.log("CalldataDraft deployed (simple) at:", address(registry));
        return registry;
    }

    /// @notice Compute the expected CREATE2 deployment address
    function computeAddress() public pure returns (address) {
        bytes32 codeHash = keccak256(type(CalldataDraft).creationCode);
